...
struct PriceTextField: View {
    ...
    init(currencySymbol: String?, placeholder: String, value: Binding<Double?>) {
        ...
        _text = State(initialValue: value.wrappedValue == nil ? "" : textFromDouble)
    }
    ...
    var body: some View {
        TextField(currencySymbol == nil ? placeholder : "\(placeholder) \(currencySymbol ?? "")", text: $text)
            ...
            .onChange(of: value) { _, newValue in
                updateTextFromValue(newValue: newValue)
            }
    }

    // MARK: - mettre à jour le texte à partir de la valeur

    func updateTextFromValue(newValue: Double?) {
